...
type ShortUrlStorage = Arc<Mutex<HashMap<String, String>>>;
// Rate limiting: IP -> (last_request_time, request_count)
type RateLimitStorage = Arc<Mutex<HashMap<String, (Instant, u32)>>>;
...
async fn main() -> Result<()> {
    ...
    info!("Loaded configuration: bind_address={}, max_file_size={}, temp_directory={:?}", 
          config.bind_address, format_size(config.max_file_size_limit), config.temp_directory);
